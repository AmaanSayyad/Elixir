...
;; stake token
;; pass in a swapr token for the ELIXIR-STX pair token and amount
(define-public (stake (amount uint))
  (begin
    (print "geyser.stake")
    (print amount)
    (if (is-ok (contract-call? 'ST3J2GVMMM2R07ZFBJDWTYEYAR8FZH5WKDTFJ9AHA.Elixir-stx-token transfer (as-contract tx-sender) amount))
...
;; unstake token
;; collect back the ELIXIR-STX pair token and ELIXIR reward
...
;; reward is amount * reward-factor * #blocks / reward-period per 1000 swapr-Elixir-wrapr token in Elixir tokens
(define-private (send-back (recipient principal) (amount uint) (blocks uint) (reward-factor uint))
  (let ((reward-amount (/ (* amount (* blocks reward-factor)) (* u1000 reward-period))))
    ...
    (if (is-ok (as-contract (contract-call? 'ST3J2GVMMM2R07ZFBJDWTYEYAR8FZH5WKDTFJ9AHA.Elixir-stx-token transfer recipient amount)))
      (if (is-ok (as-contract (contract-call? 'ST3J2GVMMM2R07ZFBJDWTYEYAR8FZH5WKDTFJ9AHA.Elixir-token transfer recipient reward-amount)))
        (ok true)
...
;; find out total amount of staked ELIXIR-WRAPR pair tokens
